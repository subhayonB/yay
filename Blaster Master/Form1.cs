...
namespace BlasterMaster
{
    ...
    //      ___ _      _   ___ _____ ___ ___    __  __   _   ___ _____ ___ ___ 
    //     | _ ) |    /_\ / __|_   _| __| _ \  |  \/  | /_\ / __|_   _| __| _ \
    //     | _ \ |__ / _ \\__ \ | | | _||   /  | |\/| |/ _ \\__ \ | | | _||   /
    //     |___/____/_/ \_\___/ |_| |___|_|_\  |_|  |_/_/ \_\___/ |_| |___|_|_\
    //
    ...
    public partial class Form1 : Form
    {
        ...
        private clsSound[] sndEffects = new clsSound[6];
        ...
        private void frmGame_Load(object sender, System.EventArgs e)
        {
            ...
            scrnBufferBmp = new Bitmap(600, 800, this.CreateGraphics());
        ...
        private void mainLoop()
        {
            ...
            while (AppRunning)
            {
                ...
                this.Invalidate(new Rectangle(0, 0, 600, 800));
                                ...
                                if (tDiversInPath < (level + 2))
                                {
                                    if (!invaders[(i + invaderBatchStartIndex) + (j * 6)].hasDockingCell())
                                    {
                                        switch (j)
                                        {
                                            ...
                                            case 1:
                                                if (invaders[(i + invaderBatchStartIndex) + (j * 6)].getX() < 525)
                                                {
        ...
        private void initNewInvaders(bool newGame)
        {
            ...
            for (j = 0; j <= 5; j++)
            {
                for (i = 0; i <= 9; i++)
                {
                    ...
                    dockRect[k].X = (i * 50) + 75;
                    dockRect[k].Y = (j * 55) + 60;
                    dockRect[k].Width = 50;
        ...
        private void doScroll()
        {
          /*  //------------------------------------------------------------------------------------------------------------------
            ...
            if (static_doScroll_timeScale >= speed)
            {
                for (int i = 0; i <= 7; i++)
                {
        ...
        private void createInvaderInstances()
        {
            ...
            for (int i = 0; i <= 59; i++)
            {
                invaders[i] = new clsInvader(-50, 0, 0, true);
            }
        }
        ...
        private void renderInfo()
        {
            ...
            // Level
            DrawText(graphicsBuffer, "LEVEL: " + Convert.ToString(level), 452, 742, 16, FontStyle.Bold, Brushes.Black);
            DrawText(graphicsBuffer, "LEVEL: " + Convert.ToString(level), 450, 740, 16, FontStyle.Bold, Brushes.White);

            // Score + high score
            DrawText(graphicsBuffer, "SCORE: " + Convert.ToString(score), 10, 10, 16, FontStyle.Bold, Brushes.White);
            DrawText(graphicsBuffer, "HIGH SCORE: " + Convert.ToString(highScore), 275, 10, 16, FontStyle.Bold, Brushes.Red);
            ...
            // Render
            DrawText(graphicsBuffer, "Shield: " + " " + s, 366, 702, 14, FontStyle.Regular, Brushes.Black);
            DrawText(graphicsBuffer, "Shield: " + " " + s, 368, 700, 14, FontStyle.Regular, Brushes.White);
        ...
        private void CheckForNewHighScore()
        {
            ...
            for (i = 0; i <= 4; i++)
            {
                if (score > Convert.ToInt32(Top5PlayerScore[i]))
                {
                    ...
                    playersName = Microsoft.VisualBasic.Interaction.InputBox("Name", "Name", "Default", -1, -1);
                    // Add new player
                    if (playersName == null)
                        {
        ...
        private void loadScoreTable()
        {
            ...
            catch
            {
                ...
                for (int i = 0; i <= 4; i++)
                {
                    Top5PlayerName[i] = "Blaster Master";
                }
        ...
        private void doTitle()
        {
            ...
            for (i = 0; i <= 4; i++)
            {
                ...
                // White type face
                DrawText(graphicsBuffer, Top5PlayerName[i], 165, 450 + (i * 30), 16, FontStyle.Regular, Brushes.White);
                DrawText(graphicsBuffer, Top5PlayerScore[i], 345, 450 + (i * 30), 16, FontStyle.Regular, Brushes.White);
            }
        ...
        private void doEndOfLevel()
        {
            ...
            if (genWorkingCounter < 100)
            {
               ...
               DrawText(graphicsBuffer, "Level Complete!", 181, 330, 22, FontStyle.Bold, Brushes.Red);
            }
        ...
        private void doStartOfLevel()
        {
            ...
            else if (genWorkingCounter < 150)
            {
              flashText("GET READY!", 189, 324, 25);          
            }
            else if (genWorkingCounter < 300)
            {
        ...
        private void initialize()
        {
            ...
            if (!sndEngineError)
            {
                try
                {
                    bkMusic[0] = new clsSound(System.IO.Directory.GetCurrentDirectory() + "\\bgm.wav");
                    sndEffects[0] = new clsSound(System.IO.Directory.GetCurrentDirectory() + "\\dive.wav");
                    sndEffects[1] = new clsSound(System.IO.Directory.GetCurrentDirectory() + "\\shoot.wav");
                    sndEffects[2] = new clsSound(System.IO.Directory.GetCurrentDirectory() + "\\enemyShot.wav");
                    sndEffects[3] = new clsSound(System.IO.Directory.GetCurrentDirectory() + "\\explosion.wav");
                    sndEffects[4] = new clsSound(System.IO.Directory.GetCurrentDirectory() + "\\pickup.wav");
        ...
        private void WipeGraphicsBuffer()
        {
            ...
            graphicsBuffer.FillRectangle(new SolidBrush(Color.Black), 0, 0, 800, 600);
